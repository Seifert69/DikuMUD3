...
void
dumbPlayLoop (cConHook * con, const char *cmd)
{
    con->PlayLoop (cmd);
}

void
dumbPressReturn (class cConHook * con, const char *cmd)
{
    con->PressReturn (cmd);
}

void
dumbMenuSelect (class cConHook * con, const char *cmd)
{
    con->MenuSelect (cmd);
}

void
dumbMudDown (class cConHook * con, const char *cmd)
{
    con->MudDown (cmd);
}
...
void cConHook::AddString (char *str)
{
    ...
    for (s = str; *s; s++)
    {
        if (ISNEWL (*s) || (strlen (m_aInputBuf) >= MAX_INPUT_LENGTH
                            - 4))
        {
            while (ISNEWL (*(s + 1)))
                s++;
...
void
Idle (cConHook * con, const char *cmd)
{
