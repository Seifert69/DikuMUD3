...
template <class T> T *t_array<T>::operator[](int i)
{
    if (i>0)
    {
        if (i<=len)
...
template <class T> t_array<T>::t_array()
{
...
template <class T> t_array<T>::~t_array<T>()
{
    for (int i=0; i<len; i++)
        delete t_ary[i];
...
template <class T> void t_array<T>::destroy()
{
    for (int i=0; i<len; i++)
        delete t_ary[i];
...
template <class T> t_array<T>::t_array(int esz)
{
    size=esz;
    esize=esz;
    len=0;
...
template <class T> void t_array<T>::init(int esz)
{
    size=esz;
    esize=esz;
    len=0;
...
template <class T> void t_array<T>::push(T *ob)
{
    if ((size - len) < 1)
    {
        size+=esize;
...
template <class T> T *t_array<T>::pop()
{
    assert(len>0) ;
    /*	if ( (size-len) > esize)
    	{
