...
void
cCombatList::add (class cCombat * Combat)
{
    if (nTop >= nMaxTop)
    {
        nMaxTop += 10;
...
void
cCombatList::sub (class cCombat * pc)
{
...
static int
combat_compare (const void *v1, const void *v2)
{
...
void
cCombatList::PerformViolence (void)
{
...
void
cCombatList::status (const class unit_data * ch)
{
...
void
cCombat::setCommand (const char *arg)
{
...
void
cCombat::changeSpeed (int delta)
{
    nWhen += delta;
}

int
cCombat::findOpponentIdx (class unit_data * target)
{
...
class unit_data *
    cCombat::FindOpponent (class unit_data * victim)
{
...
void
cCombat::add (class unit_data * victim)
{
    assert (victim);
...
void
cCombat::setMelee (class unit_data * victim)
{
    pMelee = victim;
}
...
//
void
cCombat::addOpponent (class unit_data * victim, int bMelee = FALSE)
{
...
void
cCombat::subOpponent (class unit_data * victim)
{
    if (nNoOpponents < 1)
        return;
...
void
cCombat::status (const class unit_data * ch)
{
    char buf[MAX_STRING_LENGTH];
    int i;
    ...
    sprintf (buf,
             ...
             STR (UNIT_NAME (CHAR_FIGHTING (pOwner))) : "NONE", nNoOpponents);
...
void
set_fighting (class unit_data * ch, class unit_data * vict, int bMelee)
{
    if (ch == vict)
        return;
...
/* start one char fighting another (yes, it is horrible, I know... )  */
void
add_fighting (class unit_data * ch, class unit_data * vict, int bMelee)
{
    if (ch == vict)
        return;
...
void
stop_fighting (class unit_data * ch, class unit_data * victim)
{
...
void
stat_combat (const class unit_data * ch, class unit_data * u)
{
    if (!IS_CHAR (u))
    {
