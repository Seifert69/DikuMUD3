...
void
preprocess_string (char *str, struct oracle_data *od)
{
    str_lower (str);
    str_rem (str, '\'');
    str_blank_punct (str);
...
char *
match_templ (char *input, struct template_type *tem)
{
...
void
eliza_store_memory (struct oracle_data *od, int n, int pri)
{
    assert (n < eliza_maxsubjects);

    if ((pri > '5') && (number ('5', pri) > '5'))
    {
        memmove (&od->oldkeywd[1], &od->oldkeywd[0],
                 sizeof (int) * (MAX_HISTORY - 1));
        od->oldkeywd[0] = n;
    }
}
...
/* Register 'n' as the index                            */
int
eliza_retrieve_memory (struct oracle_data *od)
{
    int mem;

    mem = od->oldkeywd[0];

    memmove (&od->oldkeywd[0], &od->oldkeywd[1],
             sizeof (int) * (MAX_HISTORY - 1));
...
/*
  ...
  */
char *
response (struct oracle_data *od, int subjno)
{
...
char *
eliza_process (struct oracle_data *od, char *s)
{
    int i, pri;
...
void
delayed_action (void *p1, void *p2)
{
...
void
set_delayed_action (class unit_data * npc, char *str)
{
    int when;
    char *cp;
...
// WHAT THE FUCK... WHY *4      memset (buf, 0, sizeof (buf) * 4);
    ...
    if (!
            (f =
                 fopen_cache (str_cc (g_cServerConfig.m_logdir, ELIZA_LOGFILE), "a+")))
        abort ();
...
int oracle (struct spec_arg *sarg)
{
    ...
    {
        if ((od->patient == NULL)
                && (!is_command (sarg->cmd, "north"))
                && (!is_command (sarg->cmd, "east"))
                && (!is_command (sarg->cmd, "west"))
                && (!is_command (sarg->cmd, "south"))
                && (!is_command (sarg->cmd, "northeast"))
                && (!is_command (sarg->cmd, "southeast"))
                && (!is_command (sarg->cmd, "northwest"))
                && (!is_command (sarg->cmd, "southwest"))
                && (!is_command (sarg->cmd, "ne"))
                && (!is_command (sarg->cmd, "se"))
                && (!is_command (sarg->cmd, "sw"))
                && (!is_command (sarg->cmd, "nw"))
                && (!is_command (sarg->cmd, "down"))
                && (!is_command (sarg->cmd, "up"))
    ...
    if (is_command (sarg->cmd, "tell") || is_command (sarg->cmd, "ask") ||
            is_command (sarg->cmd, "whisper"))
    {
        class unit_data *u;
    ...
    else if (!is_command (sarg->cmd, "say")
             && (!is_command (sarg->cmd, "shout")))
        return SFR_SHARE;
...
/* ====================================================================== */
struct template_type *
eliza_find_template (int subjno)
{
    int i;
...
void
eliza_get_template (char *buf, int subjno)
{
...
void
eliza_get_keyword (char *buf, int subjno, int priority)
{
...
void eliza_get_subjects (FILE * f)
{
    ...
    for (;;)
    {
        ...
        eliza_subjects[eliza_maxsubjects - 1].replies =
            add_name (buf, eliza_subjects[eliza_maxsubjects - 1].replies);
    }
}

void
eliza_gen_test_template (char *buf, struct template_type *tem)
{
    int j;
...
eliza_integrity (void)
{
    ...
    for (i = 1; i < eliza_maxtemplates; i++)
    {
        ...
        if ((j != -1) && (j < i))
            slog (LOG_ALL, 0, "Eliza Template: '%s' conflicts with '%s/%s'",
                  buf,
                  STR (eliza_template[j].exp[0]), STR (eliza_template[j].exp[1]));
    }
    ...
    for (i = 1; i < eliza_maxkeywords; i++)
    {
        for (j = 0; eliza_keyword[i].keyword[j]; j++)
        {
            ...
            if (k != -1)
                slog (LOG_ALL, 0, "Eliza Keyword '%s' may be shadowed by '%s/%s'",
                      buf,
    ...
    for (i = 1; i < eliza_maxkeywords; i++)
    {
        for (j = 0; eliza_keyword[i].keyword[j]; j++)
        {
            ...
            if (k != i)
                slog (LOG_ALL, 0,
                      "Eliza Keyword '%s' may be shadowed by keywd '%s'", buf,
                      STR (eliza_keyword[k].keyword[j]));
        }
    }
}
