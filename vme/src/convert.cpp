...
void
convert_free_unit (class unit_data * u)
{
...
void
free_inventory (class unit_data * u)
{
...
void
convert_inventory (class unit_data * u, class unit_data * pc,
                   int bList = FALSE)
{
    class unit_data *bla;
...
void
convert_player (class unit_data * pc)
{
    ;
}
...
/* Return TRUE if Ok. */
int
sanity_check (class unit_data * u)
{
...
int
shall_delete (class unit_data * pc)
{
    int days;
                    ...
                    ++dir_itr )
            {
                ...
                try
                {
                    ...
                    else if (dir_itr->path().filename().string().find(".")==string::npos)
                    {
                        ...
                        std::cout<<"Id ["<<PC_ID (pc) <<
                                 "]  Lvl "<<CHAR_LEVEL (pc) <<"]  "<<
                                 (	IS_MORTAL (pc) ? "   " : "ADMIN")<<
                                 " ("<<days_old (PC_TIME (pc).connect) << " days)";
                        ...
                        std::cout<<endl<<"Seconds:  "<<(PC_TIME (pc).played)<<endl;
                        std::cout<<endl<<"Seconds secs:  "<<secs<<endl;
                        std::cout<<endl<<"Hours vals:  "<<val<<endl;
                        if (ids[PC_ID (pc)])
                    ...
                    ++dir_itr )
            {
                ...
                try
                {
                    ...
                    else if (dir_itr->path().filename().string().find(".")==string::npos)
                    {
                        ...
                        std::cout<<UNIT_NAME (pc)<<" Lvl ["<<CHAR_LEVEL (pc)<<"] "<<
                                 (IS_MORTAL (pc) ? "   " : "ADMIN")<<endl;
...
void cleanup (void)
{
    ...
    if ( fs::is_directory( full_path ) )
    {
        ...
        std::cout<< "Players deleted:  "<<players_deleted<<endl<<"Players converted:  "<<players_converted<<endl;
        std::cout << "\n" << file_count << " files\n"
                  << dir_count << " directories\n"
                  << err_count << " errors\n";
    }
