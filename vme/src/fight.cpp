      ...
      if (CHAR_LEVEL(att) <= 3)
      {
         if (message)
            act("You are not old enough to do that!",
                A_ALWAYS, cActParameter(att), cActParameter(), cActParameter(def), TO_CHAR);
         return TRUE;
      }
      if (CHAR_LEVEL(def) <= 3)
      {
                    ...
                    int damage, int msg_group, int msg_number, int hit_location)
{
   ...
   if (msg)
   {
      ...
      default:
         act("Undefined hitlocation!", A_SOMEONE, cActParameter(att), cActParameter(), cActParameter(), TO_ALL);
         break;
      }
   }
   else
   {
      act("Undefined message numbers!", A_SOMEONE, cActParameter(att), cActParameter(), cActParameter(), TO_ALL);
   ...
   if (IS_PC(ch))
   {
      ...
      if (share > 0)
      {
         cact("You receive $2d experience points.",
              A_ALWAYS, cActParameter(ch), cActParameter(&share), cActParameter(), TO_CHAR, "xpgain");
      }
      else if (share == 0)
      {
         cact("You receive no experience points.",
              A_ALWAYS, cActParameter(ch), cActParameter(), cActParameter(), TO_CHAR, "xpgain");
      }
      else /* less than zero */
      {
...
static void exp_align_gain(class unit_data *ch, class unit_data *victim)
{
   ...
   {
      act("Oh dear, what a mess!", A_SOMEONE, cActParameter(victim), cActParameter(), cActParameter(), TO_ROOM);
            ...
            int bDisplay)
{
      ...
      if (!CHAR_FIGHTING(victim))
      {
         if (IS_PC(victim) && IS_IMMORTAL(victim) && IS_PC(ch))
         {
            cact("$1n ignores your feeble threats.",
                 A_ALWAYS, cActParameter(victim), cActParameter(), cActParameter(ch), TO_VICT, "miss_me");
            cact("$1n ignores $3n's feeble threats.",
                 A_ALWAYS, cActParameter(victim), cActParameter(), cActParameter(ch), TO_NOTVICT, "miss_other");
         }
         else
   ...
   case POSITION_MORTALLYW:
      ...
      act("$1n is mortally wounded!", A_SOMEONE, cActParameter(victim), cActParameter(), cActParameter(), TO_ROOM);
      break;

   case POSITION_INCAP:
      send_to_char("You are incapacitated!<br/>", victim);
      act("$1n is incapacitated!", A_SOMEONE, cActParameter(victim), cActParameter(), cActParameter(), TO_ROOM);
      break;

   case POSITION_STUNNED:
      act("$1n is stunned, but will probably regain consciousness again.",
          A_HIDEINV, cActParameter(victim), cActParameter(), cActParameter(), TO_ROOM);
      act("You're stunned, but will probably regain consciousness again.",
          A_SOMEONE, cActParameter(victim), cActParameter(), cActParameter(), TO_CHAR);
      break;

   case POSITION_DEAD:
      act("$1n is dead!", A_HIDEINV, cActParameter(victim), cActParameter(), cActParameter(), TO_ROOM);
      break;
   ...
   default: /* >= POSITION SLEEPING */
      ...
      if (dam > (max_hit / 5))
      {
         cact("That Really did HURT!", A_SOMEONE, cActParameter(victim), cActParameter(), cActParameter(), TO_CHAR,
              "hit_me");
         cact("$1n screams with agony!", A_SOMEONE, cActParameter(victim), cActParameter(), cActParameter(), TO_ROOM,
              "hit_other");
      }

      if (UNIT_HIT(victim) < (max_hit / 5))
         cact("You wish that your wounds would stop BLEEDING that much!",
              A_SOMEONE, cActParameter(victim), cActParameter(), cActParameter(), TO_CHAR, "hit_me");
      ...
      if (UNIT_HIT(obj) < 0)
      {
         if (ch)
         {
            act("Your $3N is broken by the impact!!!",
                A_ALWAYS, cActParameter(ch), cActParameter(), cActParameter(obj), TO_CHAR);
            act("$1n's $3N is broken by the impact!!!",
                A_ALWAYS, cActParameter(ch), cActParameter(), cActParameter(obj), TO_ROOM);
         }
            ...
            int bonus, int att_weapon_type, int primary, int attack)
{
   ...
   {
      cact("You fumble with your $2N!", A_ALWAYS, cActParameter(att), cActParameter(att_weapon), cActParameter(),
           TO_CHAR, "miss_me");
      cact("$1n fumbles with $1s $2N!", A_ALWAYS, cActParameter(att), cActParameter(att_weapon), cActParameter(),
           TO_ROOM, "miss_other");
   ...
   else /* A hit! */
   {
      if (att_weapon)
      {
         if (OBJ_VALUE(att_weapon, 3) == CHAR_RACE(def))
         {
            ...
            act("Your $2N glows!", A_SOMEONE, cActParameter(att), cActParameter(att_weapon), cActParameter(), TO_CHAR);
            act("$1n's $2N glows!", A_HIDEINV, cActParameter(att), cActParameter(att_weapon), cActParameter(),
                TO_ROOM);
         }
...
void melee_violence(class unit_data *ch, int primary)
{
   ...
   else
   {
      act("You get back in a fighting position, ready to fight!",
          A_SOMEONE, cActParameter(ch), cActParameter(), cActParameter(), TO_CHAR);
      act("$1n gets back in a fighting position ready to fight!",
          A_SOMEONE, cActParameter(ch), cActParameter(), cActParameter(), TO_ROOM);
   ...
   if (CHAR_AWAKE(sarg->owner))
   {
      ...
      if (scan4_ref(sarg->owner, h->victim))
      {
         ...
         act("$1n growls viciously at $3n.",
             A_SOMEONE, cActParameter(sarg->owner), cActParameter(), cActParameter(h->victim), TO_NOTVICT);
         act("$1n growls at you.",
             A_SOMEONE, cActParameter(sarg->owner), cActParameter(), cActParameter(h->victim), TO_VICT);
