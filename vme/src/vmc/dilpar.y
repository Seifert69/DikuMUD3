...
fundef   : ref
    {
        static const char *typname[] = {"void",  "unitptr", "string", "stringlist", "extraptr", "integer", "ERROR", "ERROR",  "ERROR",
                                        "ERROR", "ERROR",   "ERROR",  "ERROR",      "ERROR",    "ERROR",   "ERROR", "ERROR",  "ERROR",
                                        "ERROR", "ERROR",   "ERROR",  "ERROR",      "ERROR",    "zoneptr", "ERROR", "ERROR",  "ERROR",
                                        "ERROR", "ERROR",   "cmdptr", "ERROR",      "ERROR",    "ERROR",   "ERROR", "intlist"};
        char buf[600];
        char nbuf[255];
        char zbuf[255];
        int i;
        ...
        {
            dilfatal("Arguments not allowed on inline DIL programs, make it into a template instead.");
        }
...
dilcomplex : DILSI_IF '(' coreexp ')' ihold ahold block ihold ahold DILSI_ELS dilcomposed
    ...
    | DILSI_WHI pushbrk pushcnt defcnt '(' coreexp ')' ihold ahold dilcomposed ihold ahold defbrk popbrk popcnt
    {
        /*
         * allows both break and continue inside the composed
...
funcall:  FUNCTION '(' arginit darglist ')'
    {
        ...
        {
            char buf[256];
            sprintf(buf, "Illegal number of arguments for %s %d expected.", tmpl.xrefs[$1].name, tmpl.xrefs[$1].argc);
            dilfatal(buf);
        }
        ...
        for (i = 0; i < tmpl.xrefs[$1].argc; i++)
        {
            ...
            switch (tmpl.xrefs[$1].argt[i])
            {
                ...
                case DILV_INT:
                {
                    ...
                    {
                        sprintf(buf, "Incompatible argument types in argument %d (B)", i + 1);
                        dilfatal(buf);
                    }
                }
                break;
...
dilinst  : dilproc
    ...
    | DILSI_BRK ihold ahold
    {
        ...
        else
        {
            ...
            bwrite_ubit32(&wtmp, get_label(label_names[break_idx[break_no - 1]], $3));
        }
        $$.fst = $2;
        $$.lst = $3 + 4;
    }
    | DILSI_CNT ihold ahold
    {
        ...
        else
        {
            ...
            bwrite_ubit32(&wtmp, get_label(label_names[cont_idx[break_no - 1]], $3));
        }
        $$.fst = $2;
        $$.lst = $3 + 4;
    }
