...
int
trie_src_cmp (const void *keyval, const void *datum)
{
    return (((char *) keyval)[0] - ((struct trie_entry *) datum)->c);
}

int
trie_sort_cmp (const void *keyval, const void *datum)
{
    return (((struct trie_entry *) keyval)->c -
            ((struct trie_entry *) datum)->c);
}


struct trie_entry *
triebindex (char c, struct trie_type *t)
{
    if (t->nexts)
        return (struct trie_entry *)
               bsearch (&c, t->nexts, t->size, sizeof (struct trie_entry),
                        trie_src_cmp);
    else
        return NULL;
}
...
void
qsort_triedata (struct trie_type *t)
{
    int i;
...
int
trie_index (char c, struct trie_type *t)
{
    int i;
...
struct trie_type *
add_trienode (const char *s, struct trie_type *t)
{
    int i;
...
void
set_triedata (const char *s, struct trie_type *t, void *p, ubit1 nonabbrev)
{
    int i;
...
/* Free an entire trie */
void
free_trie (struct trie_type *t, void (*free_data) (void *))
{
    ubit8 i;
    ...
    trie_size -=
        (t->size * sizeof (struct trie_entry) + sizeof (struct trie_type));
    --trie_nodes;
    ...
    {   /* Yep, let's find next chars index */
        ...
        if (i >= 0 && del_trie (s + 1, &((*t)->nexts[i].t), free_data))
        {
            ...
            {   /* Yep, delete and confirm */
                trie_size -=
                    (sizeof (struct trie_entry) + sizeof (struct trie_type));
                trie_nodes--;
